...
export const ProductionGearModule: React.FC<ProductionGearModuleProps> = ({
  ...
  return (
    <div className="space-y-6">
      {/* Module Header Bar */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-[#0A0A0B] border border-white/10 p-6 rounded-sm">
        <div>
          <div className="flex items-center gap-2 text-[#E2B714] text-xs font-bold tracking-widest uppercase mb-1">
            <Camera className="w-4 h-4 text-[#E2B714]" /> Studio & Production Hub • Filmmaker OS
          </div>
        ...
        {/* Quick Summary Pill */}
        <div className="flex items-center gap-3 bg-white/5 border border-white/10 px-4 py-3 rounded-sm">
          <div className="text-right">
            <div className="text-[10px] text-white/40 uppercase tracking-widest font-bold">Sélection Matériel</div>
            <div className="text-lg font-mono font-black text-[#E2B714]">
              {totalGearDiscountedRental.toLocaleString('fr-MA')} MAD
            </div>
          </div>
          <div className="h-8 w-px bg-white/10"></div>
          <div className="text-right">
            <div className="text-[10px] text-white/40 uppercase tracking-widest font-bold">Équipe Tournage</div>
        ...
        {[
          ...
          return (
            <button
              ...
              className={`px-4 py-2.5 text-xs font-black uppercase tracking-wider rounded-sm flex items-center gap-2 transition-all cursor-pointer ${
                isActive
      ...
      {activeTab === 'gear' && (
        <div className="space-y-6">
          ...
          <div className="bg-white/5 border border-white/10 p-5 rounded-sm flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="space-y-1 flex-1">
              <label className="text-xs font-bold uppercase tracking-wider text-white/70 flex items-center justify-between">
                ...
                <span className="text-lg font-mono font-black text-[#E2B714]">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="px-5 py-3 bg-[#E2B714] text-black font-black text-xs uppercase tracking-wider rounded-sm hover:bg-amber-400 shadow-lg shadow-amber-500/20 flex items-center gap-2 cursor-pointer"
              >
          ...
          <form onSubmit={handleAddCustomGear} className="bg-white/5 border border-white/10 p-4 rounded-sm flex flex-wrap gap-3 items-center">
            <span className="text-xs font-bold uppercase tracking-wider text-white/60">Ajout Matériel Studio :</span>
            <input
              type="text"
              placeholder="Ex: Moniteur SmallHD 703 UltraBright..."
              value={customGearName}
              onChange={(e) => setCustomGearName(e.target.value)}
              className="flex-1 min-w-[200px] bg-black border border-white/10 text-xs text-white p-2 rounded-sm focus:border-[#E2B714]"
            />
            <input
              type="number"
              ...
              className="w-32 bg-black border border-white/10 text-xs font-mono font-bold text-[#E2B714] p-2 rounded-sm"
            />
            <button
              type="submit"
              className="px-4 py-2 bg-white/10 hover:bg-white/20 text-white font-bold text-xs uppercase rounded-sm border border-white/20"
            >
              + Ajouter
            </button>
          </form>
      ...
      {activeTab === 'crew' && (
        <div className="space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white/5 border border-white/10 p-5 rounded-sm">
            <div>
            ...
            <button
              ...
              className="px-5 py-3 bg-[#E2B714] text-black font-black text-xs uppercase tracking-wider rounded-sm hover:bg-amber-400 shadow-lg shadow-amber-500/20 flex items-center gap-2 cursor-pointer shrink-0"
            >
          ...
          <div className="space-y-2">
            {crewList.map((crew) => {
              return (
                <div
                  key={crew.id}
                  className={`p-4 rounded-sm border transition-all flex flex-wrap sm:flex-nowrap items-center justify-between gap-4 ${
                    crew.isSelected
                      ? 'bg-white/10 border-[#E2B714]'
                ...
                >
                  <div className="flex items-center gap-3 min-w-[250px]">
                    <input
                      ...
                      className="accent-[#E2B714] w-5 h-5 cursor-pointer"
                    />
                    <div>
                      <span className="text-[9px] font-black uppercase text-[#E2B714] bg-[#E2B714]/10 border border-[#E2B714]/20 px-1.5 py-0.5 rounded-sm">
                        {crew.department}
                      </span>
                  ...
                  <div className="flex items-center gap-6">
                    <div className="flex items-center gap-2">
                      <span className="text-xs text-white/40 font-bold uppercase">Jours :</span>
                      <input
                        type="number"
                        min="1"
                        max="30"
                        ...
                        className="w-16 bg-black border border-white/20 text-xs font-mono font-bold text-white p-1.5 rounded-sm text-center"
                      />
                    </div>
                    ...
                    <div className="text-right w-32 border-l border-white/10 pl-4">
                      <div className="text-[10px] text-white/40 uppercase font-bold">Total HT</div>
      ...
      {activeTab === 'callsheet' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-5 bg-white/5 border border-white/10 p-5 rounded-sm space-y-4">
            <h3 className="text-sm font-black text-white uppercase italic tracking-tight border-b border-white/10 pb-2 flex items-center gap-2">
              <Calendar className="w-4 h-4 text-[#E2B714]" /> Éditeur de Feuille de Service
            </h3>

            <div className="space-y-3 text-xs">
              <div>
                <label className="block text-white/50 font-bold mb-1">Titre du Projet Vidéo</label>
                <input
                  type="text"
                  value={activeCallSheet.projectTitle}
                  onChange={(e) => setActiveCallSheet({ ...activeCallSheet, projectTitle: e.target.value })}
                  className="w-full bg-black border border-white/10 text-white p-2 rounded-sm font-bold"
                />
              </div>

              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-white/50 font-bold mb-1">Client / Agence</label>
                  <input
                    type="text"
                    value={activeCallSheet.clientName}
                    onChange={(e) => setActiveCallSheet({ ...activeCallSheet, clientName: e.target.value })}
                    className="w-full bg-black border border-white/10 text-white p-2 rounded-sm"
                  />
                </div>
                <div>
                  <label className="block text-white/50 font-bold mb-1">Date du Tournage</label>
                  <input
                    type="date"
                    value={activeCallSheet.shootDate}
                    onChange={(e) => setActiveCallSheet({ ...activeCallSheet, shootDate: e.target.value })}
                    className="w-full bg-black border border-white/10 text-white p-2 rounded-sm font-mono"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-[#E2B714] font-bold mb-1">Heure Convocation (Call Time)</label>
                  <input
                    type="text"
                    value={activeCallSheet.callTime}
                    onChange={(e) => setActiveCallSheet({ ...activeCallSheet, callTime: e.target.value })}
                    className="w-full bg-black border border-white/10 text-[#E2B714] font-mono font-bold p-2 rounded-sm"
                  />
                </div>
                <div>
                  <label className="block text-white/50 font-bold mb-1">Coucher du Soleil (Sunset)</label>
                  <input
                    type="text"
                    value={activeCallSheet.sunsetTime}
                    onChange={(e) => setActiveCallSheet({ ...activeCallSheet, sunsetTime: e.target.value })}
                    className="w-full bg-black border border-white/10 text-white font-mono p-2 rounded-sm"
                  />
                </div>
              </div>

              <div>
                <label className="block text-white/50 font-bold mb-1">Lieu & Adresse du Décor</label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-white/50 font-bold mb-1">Réalisateur</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-white/50 font-bold mb-1">Chef Opérateur (DP)</label>
                  <input
                    type="text"
              ...
              <div>
                <label className="block text-white/50 font-bold mb-1">Notes Découpage Technique & Scènes</label>
                <textarea
                  rows={3}
                  value={activeCallSheet.scenesNotes}
                  onChange={(e) => setActiveCallSheet({ ...activeCallSheet, scenesNotes: e.target.value })}
                  className="w-full bg-black border border-white/10 text-white p-2 rounded-sm text-xs"
                />
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-7 bg-black border border-white/20 p-6 rounded-sm space-y-4">
            <div className="flex items-center justify-between border-b border-white/10 pb-3">
              <div className="flex items-center gap-2">
                <span className="w-3 h-3 bg-[#E2B714]"></span>
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-3 py-1.5 bg-[#E2B714] text-black font-black text-xs uppercase rounded-sm flex items-center gap-1 cursor-pointer"
                >
                  <Copy className="w-3.5 h-3.5" /> {copiedText === 'Call Sheet Copiée !' ? 'Copié !' : 'Copier WhatsApp'}
                </button>
                <button
                  onClick={() => window.print()}
                  className="px-3 py-1.5 bg-white/10 text-white font-bold text-xs uppercase rounded-sm border border-white/20 flex items-center gap-1"
                >
            ...
            <div className="bg-[#0A0A0B] border border-white/10 p-6 space-y-4 text-xs font-mono">
              <div className="flex justify-between items-start border-b border-white/10 pb-4">
                <div>
                  <h2 className="text-xl font-black text-[#E2B714] uppercase tracking-wider">{activeCallSheet.projectTitle}</h2>
                  <p className="text-white/60">CLIENT : {activeCallSheet.clientName}</p>
                </div>
                <div className="text-right">
                  <div className="text-2xl font-black text-white bg-white/10 px-3 py-1 border border-[#E2B714]">
                    {activeCallSheet.callTime}
                  </div>
                  <div className="text-[10px] text-[#E2B714] uppercase font-bold mt-1">CALL TIME ÉQUIPE</div>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-4 border-b border-white/10 pb-4">
                <div>
                  <span className="text-white/40 block text-[10px] uppercase">DATE & HORAIRES</span>
                  <div className="text-white font-bold">{activeCallSheet.shootDate}</div>
                  <div className="text-white/60 text-[11px]">Coucher du Soleil : {activeCallSheet.sunsetTime}</div>
                </div>
                <div>
                  <span className="text-white/40 block text-[10px] uppercase">ADRESSE DU DÉCOR</span>
                  <div className="text-white font-bold flex items-center gap-1">
                    <MapPin className="w-3 h-3 text-[#E2B714]" /> {activeCallSheet.locationAddress}
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 border-b border-white/10 pb-4">
                <div>
                  <span className="text-white/40 block text-[10px] uppercase">RÉALISATEUR</span>
                  <div className="text-white font-bold">{activeCallSheet.directorName}</div>
                </div>
                <div>
                  <span className="text-white/40 block text-[10px] uppercase">DIRECTEUR PHOTO (DP)</span>
                  <div className="text-white font-bold">{activeCallSheet.dpName}</div>
                </div>
              </div>

              <div>
                <span className="text-white/40 block text-[10px] uppercase mb-1">PROGRAMME ET DECOUPAGE DES SCÈNES</span>
                <div className="bg-black p-3 border border-white/10 text-white/80 whitespace-pre-wrap leading-relaxed">
                  {activeCallSheet.scenesNotes}
                </div>
              </div>
            </div>
        ...
        <div className="space-y-6">
          <div className="bg-white/5 border border-white/10 p-5 rounded-sm">
            <h3 className="text-lg font-black text-white uppercase italic tracking-tight flex items-center gap-2">
              <Shield className="w-5 h-5 text-[#E2B714]" /> Bibliothèque de Clauses Juridiques & Cession de Droits Maroc
            </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {legalClauses.map((clause) => {
              return (
                <div key={clause.id} className="bg-white/5 border border-white/10 p-5 rounded-sm space-y-3 relative">
                  <div className="flex items-center justify-between">
                    <span className="text-[9px] font-black uppercase text-[#E2B714] bg-[#E2B714]/10 border border-[#E2B714]/30 px-2 py-0.5 rounded-sm">
                      {clause.category}
                    </span>

                    <button
                      onClick={() => handleCopy(clause.content, clause.id)}
                      className="px-2.5 py-1 bg-white/10 hover:bg-[#E2B714] hover:text-black text-white font-bold text-[10px] uppercase rounded-sm transition-all flex items-center gap-1"
                    >
                  ...
                  <p className="text-xs text-white/70 bg-black/60 p-3 rounded-sm border border-white/5 leading-relaxed font-mono">
                    "{clause.content}"
                  </p>
                </div>
              );
