...
export const FinancialDashboardModule: React.FC<FinancialDashboardModuleProps> = ({
  ...
  // Direct Revenue Filter & View State
  const [directViewMode, setDirectViewMode] = useState<'table' | 'cards'>('table');
  const [directStatusFilter, setDirectStatusFilter] = useState<'ALL' | 'paye' | 'en_attente'>('ALL');
  const [directSearchQuery, setDirectSearchQuery] = useState<string>('');

  // Direct Revenue Quick Action Helpers
  const handleToggleDirectStatus = (item: DirectRevenueItem) => {
    const updated: DirectRevenueItem = {
      ...item,
      status: item.status === 'paye' ? 'en_attente' : 'paye',
    };
    if (onSaveDirectRevenue) {
      onSaveDirectRevenue(updated);
    }
  };

  const handleDeleteDirectItem = (id: string, title?: string) => {
    if (window.confirm(`Voulez-vous vraiment supprimer "${title || 'ce forfait / revenu direct'}" ? Cette action est irréversible.`)) {
      if (onDeleteDirectRevenue) {
        onDeleteDirectRevenue(id);
      }
    }
  };
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400">Missions hebdomadaires, mensuelles ou directes enregistrées ({directRevenues.length})</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={() => handleOpenAddModal()}
                  className="px-3 py-1.5 bg-emerald-500/20 text-emerald-300 hover:bg-emerald-500/30 border border-emerald-500/30 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all"
                >
                  <Plus className="w-3.5 h-3.5" /> + Nouveau
                </button>
                <button
                  onClick={() => setActiveTab('direct_revenues')}
                  className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-amber-400 text-xs font-bold rounded-xl flex items-center gap-1 transition-all"
                >
                  Gérer tous les forfaits ({directRevenues.length}) →
                </button>
              </div>
            </div>

            {directRevenues.length === 0 ? (
              <div className="text-center py-6 border border-dashed border-slate-800 rounded-xl text-slate-400 text-xs space-y-2">
                <div>Aucun revenu direct pour l'instant.</div>
                <button
                  onClick={() => handleOpenAddModal()}
                  className="px-3 py-1.5 bg-amber-500 text-slate-950 rounded-lg text-xs font-bold inline-flex items-center gap-1"
                >
                  <Plus className="w-3.5 h-3.5" /> Ajouter un premier forfait
                </button>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                {directRevenues.slice(0, 6).map((item) => (
                  <div key={item.id} className="bg-slate-950 border border-slate-800/80 p-4 rounded-xl space-y-3 relative group hover:border-slate-700 transition-all">
                    <div className="flex items-center justify-between gap-2">
                      <span className="text-[10px] font-mono uppercase px-2 py-0.5 rounded bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 font-bold">
                        {item.frequency === 'weekly' ? '⚡ Hebdo' : item.frequency === 'monthly' ? '📅 Mensuel' : '🎯 Ponctuel'}
                      </span>
                      <div className="flex items-center gap-2">
                        <button
                          type="button"
                          onClick={() => handleToggleDirectStatus(item)}
                        ...
                        >
                          {item.status === 'paye' ? '✓ Encaissé' : '⏳ En attente'}
                        </button>
                    ...
                    <div>
                      <h5 className="text-xs font-bold text-white truncate" title={item.title}>{item.title}</h5>
                      <div className="text-[11px] text-slate-400 flex items-center justify-between mt-0.5">
                        <span className="truncate">{item.clientCompany || item.clientName}</span>
                        <span className="text-[10px] text-slate-500 shrink-0 font-mono">
                          {item.amountMAD.toLocaleString('fr-MA')} MAD × {item.occurrencesCount || 1}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between pt-2.5 border-t border-slate-800/80 text-xs">
                      ...
                      <div className="flex items-center gap-1.5">
                        <button
                          ...
                          className="px-2.5 py-1 bg-slate-900 hover:bg-amber-500/20 text-slate-300 hover:text-amber-300 rounded-lg text-[11px] font-bold border border-slate-800 transition-all flex items-center gap-1 cursor-pointer"
                          title="Modifier ce forfait"
                        >
                          <Edit className="w-3 h-3 text-amber-400" /> Modifier
                        </button>
                        <button
                          type="button"
                          onClick={() => handleDeleteDirectItem(item.id, item.title)}
                          className="p-1.5 bg-slate-900 hover:bg-rose-500/20 text-slate-400 hover:text-rose-400 rounded-lg border border-slate-800 transition-all cursor-pointer"
        ...
        <div className="space-y-6">
          ...
          <div className="bg-emerald-950/20 border border-emerald-500/30 p-5 rounded-2xl space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-300">
                    Enregistrez, modifiez et gérez vos travaux réguliers sans formalités administratives (sans devis/facture papier).
                  </p>
                </div>
              </div>
              ...
              <button
                onClick={() => handleOpenAddModal()}
                className="px-4 py-2.5 bg-gradient-to-r from-emerald-500 to-emerald-400 hover:brightness-110 text-slate-950 font-black text-xs uppercase tracking-wider rounded-xl shadow-lg shadow-emerald-500/20 flex items-center gap-2 cursor-pointer transition-all"
              >
                <Plus className="w-4 h-4 text-slate-950" />
                Nouveau Forfait / Revenu Direct
              </button>
            </div>
          ...
          {/* Direct Revenue Main Container: Filter, Search & View Switcher */}
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4">
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-slate-800 pb-4">
              <div>
                <h4 className="text-sm font-bold text-white flex items-center gap-2">
                  <Wallet className="w-4 h-4 text-emerald-400" />
                  Journal des Entrées Directes & Forfaits
                </h4>
                <p className="text-xs text-slate-400">
                  {directRevenues.length} contrat(s) au total • Modifier, supprimer et ajuster les règlements en 1 clic
                </p>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-2.5">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="bg-slate-950 border border-slate-800 text-xs text-slate-200 pl-8 pr-3 py-1.5 rounded-xl w-48 sm:w-56 focus:border-amber-500 focus:outline-none"
                  />
                  {directSearchQuery && (
                    <button
                ...
                <div className="flex items-center bg-slate-950 p-0.5 rounded-xl border border-slate-800 text-xs">
                  <button
                    onClick={() => setDirectStatusFilter('ALL')}
                    className={`px-2.5 py-1 rounded-lg font-bold transition-all cursor-pointer ${
                      directStatusFilter === 'ALL' ? 'bg-amber-500 text-slate-950 font-black' : 'text-slate-400 hover:text-white'
                    }`}
                  >
                    Tous ({directRevenues.length})
                  </button>
                  <button
                    onClick={() => setDirectStatusFilter('paye')}
                    className={`px-2.5 py-1 rounded-lg font-bold transition-all cursor-pointer ${
                      directStatusFilter === 'paye' ? 'bg-emerald-500 text-slate-950 font-black' : 'text-slate-400 hover:text-white'
                    }`}
                  >
                    Encaissés ({directRevenues.filter(r => r.status === 'paye').length})
                  </button>
                  <button
                    onClick={() => setDirectStatusFilter('en_attente')}
                    className={`px-2.5 py-1 rounded-lg font-bold transition-all cursor-pointer ${
                      directStatusFilter === 'en_attente' ? 'bg-amber-500 text-slate-950 font-black' : 'text-slate-400 hover:text-white'
                    }`}
                  >
                    En attente ({directRevenues.filter(r => r.status === 'en_attente').length})
                  </button>
                </div>
                ...
                <div className="flex items-center bg-slate-950 p-0.5 rounded-xl border border-slate-800 text-xs">
                  <button
                    onClick={() => setDirectViewMode('table')}
                    className={`p-1.5 rounded-lg transition-all cursor-pointer ${
                      directViewMode === 'table' ? 'bg-slate-800 text-white' : 'text-slate-400 hover:text-white'
                    }`}
                  ...
                  <button
                    onClick={() => setDirectViewMode('cards')}
                    className={`p-1.5 rounded-lg transition-all cursor-pointer ${
                      directViewMode === 'cards' ? 'bg-slate-800 text-white' : 'text-slate-400 hover:text-white'
                    }`}
                ...
                <button
                  onClick={() => handleOpenAddModal()}
                  className="px-3 py-1.5 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-black text-xs rounded-xl flex items-center gap-1 transition-all cursor-pointer"
                >
            ...
            {(() => {
              const itemsToShow = directRevenues.filter((item) => {
                const matchesStatus = directStatusFilter === 'ALL' || item.status === directStatusFilter;
                const query = directSearchQuery.toLowerCase().trim();
                const matchesSearch =
                  !query ||
                  item.title.toLowerCase().includes(query) ||
                  item.clientName.toLowerCase().includes(query) ||
                  (item.clientCompany && item.clientCompany.toLowerCase().includes(query)) ||
                ...
                return (
                  <div className="text-center py-12 border border-dashed border-slate-800 rounded-xl space-y-3">
                    ...
                    <div className="text-slate-300 text-sm font-bold">
                      {directSearchQuery || directStatusFilter !== 'ALL'
                        ? 'Aucune entrée ne correspond à vos filtres de recherche.'
                        : 'Aucun revenu direct enregistré pour l\'instant.'}
                    </div>
                    ...
                    <div className="flex items-center justify-center gap-2">
                      {(directSearchQuery || directStatusFilter !== 'ALL') && (
                        <button
                          ...
                          className="px-3 py-1.5 bg-slate-800 text-slate-300 hover:text-white rounded-xl text-xs font-bold cursor-pointer"
                        >
                      ...
                      <button
                        onClick={() => handleOpenAddModal()}
                        className="px-4 py-2 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-black text-xs rounded-xl cursor-pointer"
                      >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 pt-2">
                    {itemsToShow.map((item) => {
                      ...
                      return (
                        <div
                          key={item.id}
                          className="bg-slate-950 border border-slate-800 rounded-2xl p-4 space-y-3.5 hover:border-slate-700 transition-all flex flex-col justify-between"
                        >
                          <div className="space-y-2.5">
                            {/* Card Header: Frequency Badge + Status Toggle */}
                            <div className="flex items-center justify-between gap-2">
                              <span className="text-[10px] font-mono uppercase px-2.5 py-0.5 rounded-lg bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 font-bold">
                              ...
                              >
                                {item.status === 'paye' ? (
                                  <>
                                    <Check className="w-3 h-3 text-emerald-400" /> Encaissé
                                  </>
                                ) : (
                                  <>
                            ...
                            <div className="bg-slate-900/90 p-2.5 rounded-xl border border-slate-800/80 flex items-center justify-between text-xs">
                              <div>
                                <div className="text-slate-400 text-[10px]">Client / Entreprise</div>
                              ...
                              <div className="text-right font-mono">
                                <div className="text-slate-400 text-[10px] uppercase">Règlement</div>
                            ...
                            <div className="bg-slate-900/60 p-3 rounded-xl border border-slate-800/60 flex items-center justify-between">
                              <div>
                                <div className="text-[10px] text-slate-400 font-mono">
                                  {item.amountMAD.toLocaleString('fr-MA')} MAD × {item.occurrencesCount || 1} versement(s)
                                </div>
                                <div className="text-[10px] text-slate-500">
                                  {item.frequency === 'weekly' ? 'Tarif par semaine' : item.frequency === 'monthly' ? 'Tarif par mois' : 'Montant forfait'}
                                </div>
                              </div>
                            ...
                            {item.notes && (
                              <div className="text-[11px] text-amber-300/80 bg-amber-950/20 border border-amber-500/20 p-2 rounded-lg italic">
                                💬 {item.notes}
                              </div>
                            )}
                          </div>

                          {/* Card Actions Footer: Modifier & Supprimer */}
                          <div className="flex items-center justify-between pt-3 border-t border-slate-800 gap-2">
                            <button
                              type="button"
                              onClick={() => handleOpenEditModal(item)}
                              className="flex-1 py-2 bg-slate-900 hover:bg-amber-500/20 text-slate-300 hover:text-amber-300 rounded-xl text-xs font-bold border border-slate-800 hover:border-amber-500/40 transition-all flex items-center justify-center gap-1.5 cursor-pointer"
                            >
                              <Edit className="w-3.5 h-3.5 text-amber-400" /> Modifier
                            </button>

                            <button
                              type="button"
                              onClick={() => handleDeleteDirectItem(item.id, item.title)}
                              className="px-3 py-2 bg-slate-900 hover:bg-rose-500/20 text-slate-400 hover:text-rose-300 rounded-xl text-xs font-bold border border-slate-800 hover:border-rose-500/40 transition-all flex items-center justify-center gap-1 cursor-pointer"
                  ...
                  <table className="w-full text-left text-xs border-collapse">
                    <thead>
                      <tr className="border-b border-slate-800 text-slate-400 uppercase text-[10px] tracking-wider bg-slate-950/40">
                        ...
                        <th className="py-3 px-4 font-bold text-right">Total Perçu (MAD)</th>
                        <th className="py-3 px-4 font-bold text-center">Règlement</th>
                        <th className="py-3 px-4 font-bold text-center">Statut</th>
                        <th className="py-3 px-4 font-bold text-center">Actions &amp; Gestion</th>
                      </tr>
                    </thead>
                        ...
                        return (
                          <tr key={item.id} className="hover:bg-slate-800/40 transition-colors">
                            <td className="py-3.5 px-4">
                              ...
                              {item.notes && (
                                <div className="text-[10px] text-amber-400/80 italic mt-0.5 truncate max-w-xs" title={item.notes}>
                                  {item.notes}
                                </div>
                              )}
                            </td>
                            ...
                            <td className="py-3.5 px-4">
                              {item.frequency === 'weekly' ? (
                                ...
                                <span className="px-2 py-0.5 rounded bg-purple-500/10 text-purple-400 border border-purple-500/20 font-bold text-[11px]">
                                  🎯 Ponctuel
                                </span>
                              )}
                            </td>
                            ...
                            <td className="py-3.5 px-4 text-center">
                              <button
                                type="button"
                                onClick={() => handleToggleDirectStatus(item)}
                                className={`px-2.5 py-1 rounded-lg font-bold border transition-all cursor-pointer text-[11px] inline-flex items-center gap-1 ${
                                  item.status === 'paye'
                                    ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30 hover:bg-emerald-500/30'
                                    : 'bg-amber-500/20 text-amber-400 border-amber-500/30 hover:bg-amber-500/30'
                                }`}
                                title="Cliquer pour changer le statut (Payé / En attente)"
                              >
                                {item.status === 'paye' ? (
                                  <>
                                    <Check className="w-3 h-3 text-emerald-400" /> Encaissé
                            ...
                            <td className="py-3.5 px-4 text-center">
                              <div className="flex items-center justify-center gap-1.5">
                                <button
                                  type="button"
                                  onClick={() => handleOpenEditModal(item)}
                                  className="px-2.5 py-1 bg-slate-800 hover:bg-amber-500/20 text-slate-300 hover:text-amber-300 border border-slate-700 hover:border-amber-500/40 rounded-lg text-xs font-bold transition-all flex items-center gap-1 cursor-pointer"
                                  title="Modifier ce forfait"
                                >
                                  <Edit className="w-3.5 h-3.5 text-amber-400" /> Modifier
                                </button>
                                <button
                                  type="button"
                                  onClick={() => handleDeleteDirectItem(item.id, item.title)}
                                  className="p-1.5 bg-slate-800 hover:bg-rose-500/20 text-slate-400 hover:text-rose-400 border border-slate-700 hover:border-rose-500/40 rounded-lg transition-all cursor-pointer"
                                  title="Supprimer ce forfait"
                                >
                                  <Trash2 className="w-3.5 h-3.5" />
                                </button>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-3 border-t border-slate-800">
                {editingItemId ? (
                  <button
                    type="button"
                    onClick={() => {
                      handleDeleteDirectItem(editingItemId, formTitle);
                      setIsModalOpen(false);
                    }}
                    className="w-full sm:w-auto px-4 py-2.5 bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 hover:text-rose-300 border border-rose-500/30 rounded-xl font-bold flex items-center justify-center gap-2 cursor-pointer transition-all"
                  >
                ...
                <div className="flex items-center gap-3 w-full sm:w-auto justify-end">
                  <button
                    type="button"
                    onClick={() => setIsModalOpen(false)}
                    className="flex-1 sm:flex-none px-4 py-2.5 bg-slate-800 text-slate-300 hover:text-white rounded-xl font-bold cursor-pointer"
                  >
                    Annuler
                  </button>
                  <button
                    type="submit"
                    className="flex-1 sm:flex-none px-5 py-2.5 bg-gradient-to-r from-emerald-500 to-emerald-400 text-slate-950 font-black rounded-xl shadow-lg shadow-emerald-500/20 hover:brightness-110 flex items-center justify-center gap-2 cursor-pointer"
                  >
                    <Check className="w-4 h-4" />
                    {editingItemId ? 'Mettre à jour le Forfait' : 'Enregistrer le Revenu'}
                  </button>
                </div>
              </div>
            </form>
